...
личное сообщение с двумя кнопками: "Я уже здесь" (регистрирует возврат так
же, как `/afk back`) и "Мне нужно ещё полчаса" (сдвигает расчётное время на
30 минут, в канвасе у записи появляется пометка `needs 30 minutes` и
будильник в `Back in` — до фактического возврата; наблюдателям из
`/afk wait` тоже приходит уведомление о продлении).
