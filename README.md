...
- `/afk 1.5h [комментарий]` — отметить, что отошёл; пишет/обновляет запись в
  Canvas (можно сразу в нескольких — см. `CANVAS_IDS`)
- `/afk back` — досрочное возвращение
- `/afk lang [ru|en|pl|uk]` — выбрать язык общения с ботом (без аргумента —
  показывает текущий)
- `/afk wait @user [@user2 ...]` — попросить бота черкнуть в личку, когда
  указанные люди отметят возвращение

Формат длительности: первое число в тексте (`<=12` считается часами, `>12` —
минутами; `,` и `.` как разделитель дробной части); всё после числа —
...
можно явно переопределить командой `/afk lang ru|en|pl|uk`. Канвас (общий для
всех, кто на него смотрит) — всегда на английском вне зависимости от языка
бота у конкретного человека.
